import { OmitType, PartialType, PickType } from "@nestjs/swagger";
import { IsNumber, IsOptional, IsString } from "class-validator";
import { Preemption } from "../entities/preemption.entity";

export class CreatePreemptionDto extends OmitType(Preemption, ['id','status'] as const) { }
export class UpdateAllPreemptionDto extends Preemption { }
export class UpdatePreemptionDto extends PartialType(CreatePreemptionDto) { }
export class UpdatePreemptionStatusDto extends PickType(Preemption, ['status']) { }
export class ListPreemptionDto extends PartialType(Preemption) { }
export class ListMyPreemptionDto {
    @IsOptional()
    @IsString()
    status?: string
}